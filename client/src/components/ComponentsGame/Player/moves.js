...
export function changeDirectionPlayers(state, { direction }) {
  return state.players.reduce(
    (lastState, player, index) =>
      changeDirection(lastState, direction, player, index),
    state
  );
}

// Cambia la dirección del movimiento del jugador
export function changeDirection(state, { direction }, player, index) {
  console.log(direction);
  console.log(player);
  console.log(index);
  const oldPolarity = setPolarity(state.players.direction);
  console.log(oldPolarity);
...
export function walking(state, { time = Date.now() } = {}) {
  ...
  const playerWalking = updatePlayers(
...
export function updatePlayer(state, time, player, index) {
  const newVector = getNewPlayerVector(player, time);
  ...
  /*const eatentabsIndex = getEatentabs(
    state.tabs,
    state.player,
    newVector.position
  );*/
...
export function updatePlayers(state, time) {
  return state.players.reduce(
    (lastState, player, index) => updatePlayer(lastState, time, player, index),
    state
  );
}
...
export function continueMoving(coord3, coord1, position, tolerance) {
  const snap = coord1
    ? Math.ceil(position[coord3])
    : Math.floor(position[coord3]);
...
export function getChangedVector(
  oldPosition,
  newPosition,
  oldDirection,
  newDirection,
  ...
  if (!(old0 === new0 && movedDistanceBeforeTurn > movedDistance)) {
    const { coord1: newCoord1, coord3: newCoord3, coord2 } = setPolarity(
      newDirection
    );
    ...
    const pathHit = path.findIndex(
      (limits) =>
        ...
        (1 - newCoord1) * newPosition[newCoord3] >=
          (limits[0] - coord2) * (1 - newCoord1) &&
    ...
    if (pathHit > -1) {
      ...
      changedVector[newCoord3] +=
        coord2 * (movedDistance - movedDistanceBeforeTurn);
